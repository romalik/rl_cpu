...
class MCompiler {
    ...
    typedef std::map<std::string, std::vector<std::pair<std::string, int> > > MacroVector;
    MacroVector macros;
...
public:
    ...
    void compile() {
        ...
        while(std::getline(file, line)) {
            ...
            } else if(wordsVector[0] == "macro") {
		printf("Macro section\n");
		currentState = MACRO_STATE;
		...
		if(currentState == MACRO_STATE) {
		    printf("Macro section end. Collected %d macros:\n", macros.size());
		    for(MacroVector::iterator it = macros.begin(); it != macros.end(); ++it) {
			printf("Macro name \"%s\": ", it->first.c_str());
			for(int j = 0; j<(*it).second.size(); j++) {
				printf(" %s = %d; ", (*it).second[j].first.c_str(), (*it).second[j].second);
			}
			printf("\n");
		    }
		}
		...
		if(currentState == MACRO_STATE) {
		        std::string macroName = wordsVector[0];
			wordsVector.erase(wordsVector.begin());
			while(wordsVector.size() > 1) {
				macros[macroName].push_back(std::pair<std::string, int>(wordsVector[0], atoi(wordsVector[1].c_str())));
				wordsVector.erase(wordsVector.begin());
				wordsVector.erase(wordsVector.begin());
			}
			...
			MacroVector::iterator macroIterator = macros.find(wordsVector[i]);
			if(macroIterator != macros.end()) {
			   for(int j = 0; j<macroIterator->second.size(); j++) {
			   
                        	DefVector::iterator bitPos = definitions.find(macroIterator->second[j].first);
                        	if(bitPos == definitions.end()) {
                            		printf("Macro parsing: definition %s not found!\n", macroIterator->second[j].first.c_str());
                            		break;
                        	}
                        	inst[bitPos->second.first] = macroIterator->second[j].second;
			   }
			} else {
