...
static const uint8_t fp_do[][NSTATE] = {
    ...
    {F_NADA, F_NADA, F_NADA, F_QUIT, F_QUIT, F_QUIT, F_QUIT,
     F_QUIT}, /* see '.' */
    {F_QUIT, F_QUIT, F_NADA, F_QUIT, F_NADA, F_QUIT, F_QUIT,
     F_QUIT}, /* see e/E */
    {F_SIGN, F_QUIT, F_QUIT, F_QUIT, F_QUIT, F_ESIGN, F_QUIT,
     F_QUIT}, /* see sign */
};
/* given transition,state what is new state? */
static const uint8_t fp_ns[][NSTATE] = {
    {FS_DIGS, FS_DIGS, FS_DIGS, FS_DD, FS_DD, FS_EDIGS, FS_EDIGS,
     FS_EDIGS}, /* see digit */
    {
     FS_DOT, FS_DOT, FS_DD,
    }, /* see '.' */
...
static const uint8_t fp_sval[NSTATE] = {0, 0, 1, 0, 1, 0, 0, 1};
    ...
    while (c > 0) {
        ...
        fmtnxt:
            ...
            numfmt:
                ...
                digits[base] = '\0';
                p = ((unsigned char *)strchr((char *)digits, toupper(c)));
            ...
            strproc:
                ...
                if (width) {
                    q = ((unsigned char *)strchr((char *)delim, c));
                    if ((c < 1) || lval == (q == 0)) {
                        if (endnull)
                            *p = '\0';
                        goto done;
