...
#define BUFSIZE         64*8     /* uzix buffer/block size */
...
struct __stdio_file {
        uint8_t   *bufpos;        /* the next byte to write to or read from */
        uint8_t   *bufread;       /* the end of data returned by last read() */
        uint8_t   *bufwrite;      /* highest address writable by macro */
        uint8_t   *bufstart;      /* the start of the buffer */
        uint8_t   *bufend;        /* the end of the buffer; ie the byte after
