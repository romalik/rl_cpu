 ...
 */
static int prtfld(FILE *op, size_t maxlen, size_t ct, unsigned char *buf,
...
int _vfnprintf(FILE *op, size_t maxlen, const char *fmt, va_list ap) {
    ...
    while (*fmt) {
        ...
        fmtnxt:
            ...
            case 'i':
                ptmp = __ltostr(
                    (long)((lval) ? va_arg(ap, long) : va_arg(ap, int)), 10);
                goto printit;
            ...
            usproc:
                val =
                    lval ? va_arg(ap, unsigned long) : va_arg(ap, unsigned int);
            ...
            chrpad:
                sign = '\0';
            printit:
                cnt += prtfld(op, maxlen, cnt, (unsigned char *)ptmp, ljustf,
            ...
            case 'E':
            case 'G':
                _fnum(va_arg(ap, double), *fmt, preci, ptmp);
                /* double arg;
                   char fmt;  (e/f/g/E/G)
