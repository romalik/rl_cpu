...
void copyBanks(unsigned int dest, unsigned int src) {
  ...
  while(copied < 0x8000) {
    di();
    mm_memcpy(forkBuffer, (unsigned int *)(0x8000+copied), FORK_BUFFER_SIZE, src);
    mm_memcpy((unsigned int *)(0x8000+copied), forkBuffer, FORK_BUFFER_SIZE, dest);
      ...
      if(kernelTaskQueue[i].type != KERNEL_TASK_NONE) {
        if(kernelTaskQueue[i].type == KERNEL_TASK_FORK) {
          ...
          if(findProcByPid(kernelTaskQueue[i].src, &p)) {
            ...
            BANK_SEL = newBank;
            sStruct = (struct ForkSyscallStruct *)(*((size_t *)(newProcess->ap))); //syscall struct pointer sits in first arg in arg space
            sStruct->pid = 0;

            //set child pid as retval for parent
            BANK_SEL = currentBank;
            sStruct = (struct ForkSyscallStruct *)(*((size_t *)(p->ap))); //syscall struct pointer sits in first arg in arg space
            sStruct->pid = newPid;
